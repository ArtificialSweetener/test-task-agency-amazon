    ...
    public Map<Long, SPCampaignStatistic> getSPCampaignsStatistics(
        ...
        for (SPCampaignReport report : reports) {
            ...
            campaignAnalyticMap.compute(campaignId, (k, v) -> {
                ...
                v.setClicks(v.getClicks() + report.getClicks());
                v.setImpressions(v.getImpressions() + report.getImpressions());
                v.setCost(v.getCost().add(report.getCost()));
                v.setCostPerClick(finalRound(calculateCPC(v.getCost(), v.getClicks())));
                v.setPurchases(v.getPurchases() + report.getPurchases14d());
                return v;
            });
        }
        ...
        for (
                SPCampaign campaign : allEnabledCampaigns) {
        ...
        finalCampaignAnalyticMap.values().

                forEach(SPCampaignStatistic::finalise);
